...
function kurulum({ kanal = true, kosu = null, pano = true } = {}) {
  ...
  if (kosu) writeFileSync(join(kok, 'tools', 'sevk', '.kosu-acik'), kosu);
  return kok;
}
...
const KOSU_SATIRI = (damga) => `KOSU-E5\tKT-900\tyapim\tbassiz\ttatbikat\ndamga\t${damga}\n`;
...
test('haber: dört olayın gövdesi şablondan kurulur (prova)', () => {
  const kok = kurulum();
  const a = haber(kok, ['--olay', 'kosu-basladi', '--kosu', 'K1', '--kutu', 'KT-900',
                        '--tur', 'yapim', '--kip', 'bassiz', '--sinif', 'gercek', '--prova']);
  assert.equal(a.status, 0);
  assert.match(a.stdout, /PROVA\tkosu-basladi\tTEMIZ/);
  assert.match(a.stdout, /Koşu açıldı: K1/);
  assert.match(a.stdout, /KEEL DUR/, 'durdurma yolu ilk postada yazılı olmalı');

  const b = haber(kok, ['--olay', 'kosu-bitti', '--kosu', 'K1', '--kutu', 'KT-900',
                        '--blok1', 'iki koşu', '--blok2', 'kuyruk boş', '--blok3', 'durdu', '--prova']);
  assert.match(b.stdout, /GECE NE OLDU\niki koşu/);
  assert.match(b.stdout, /SENDE BEKLEYEN\nkuyruk boş/);
  assert.match(b.stdout, /ŞİMDİ NE YAPIYOR\ndurdu/);

  const c = haber(kok, ['--olay', 'catal-bekliyor', '--kosu', 'K1', '--kutu', 'KT-900',
  ...
  const d = haber(kok, ['--olay', 'alarm', '--cins', 'sisme', '--kosu', 'K1', '--kutu', 'KT-900',
...
test('haber: süzgeç eşleşmesi postayı DURDURUR; değer sansürlü şablona SIZMAZ', () => {
  const kok = kurulum();
  const kart = luhnKart();
  const r = haber(kok, ['--olay', 'alarm', '--cins', 'kirmizi', '--kosu', 'K1', '--kutu', 'KT-900',
...
test('haber: süzgeç KOŞAMAZSA da temiz sayılmaz (fail-closed)', () => {
  ...
  const r = haber(kok, ['--olay', 'alarm', '--cins', 'kanal', '--kosu', 'K1', '--kutu', 'KT-900',
...
test('DUR hat-1: .dur varken yeni alt-ajan AÇILMAZ (frenleme hattı)', () => {
  const kok = kurulum({ kosu: KOSU_SATIRI(new Date().toISOString()) });
...
// ── 6 · Watchdog / nabız ──────────────────────────────────────────────────────────────────
test('nabız: koşu yokken SESSİZ (sıradan günler etkilenmez) ama canlılık damgası basar', () => {
  const kok = kurulum();
  const r = kos(kok, 'nabiz.sh');
  assert.equal(r.status, 0);
  assert.equal(r.stdout, '', 'koşu yoksa hiçbir şey söylemez');
  assert.ok(existsSync(join(kok, 'tools', 'sevk', '.nabiz-son')), 'canlılık damgası her turda basılır');
  assert.equal(gunluk(kok).length, 0, 'koşu yokken günlüğe satır düşmez');
});

test('nabız durum (b): koşu AÇIK ama HİÇ nabız yok → sessizlik alarmı', () => {
  const eski = new Date(Date.now() - 90 * 60000).toISOString();
  const kok = kurulum({ kosu: KOSU_SATIRI(eski) });
  appendFileSync(join(kok, '00_pano', 'zarf-gunlugu.jsonl'),
    JSON.stringify({ surum: 1, ts: eski, kosu: 'KOSU-E5', tip: 'kosu-acilis' }) + '\n');
  kos(kok, 'nabiz.sh');
  ...
  assert.ok(existsSync(join(kok, 'tools', 'sevk', '.kosu-acik')), 'watchdog koşuyu DİRİLTMEZ ve göstergeyi silmez');
});
...
test('nabız durum (a): nabız var ama eşik aşıldı → sessizlik alarmı', () => {
  ...
  const kok = kurulum({ kosu: KOSU_SATIRI(eski) });
  const y = join(kok, '00_pano', 'zarf-gunlugu.jsonl');
  appendFileSync(y, JSON.stringify({ surum: 1, ts: eski, kosu: 'KOSU-E5', tip: 'kosu-acilis' }) + '\n');
  appendFileSync(y, JSON.stringify({ surum: 1, ts: eski, kosu: 'KOSU-E5', tip: 'nabiz' }) + '\n');
  kos(kok, 'nabiz.sh');
  const a = gunluk(kok).filter((j) => j.tip === 'alarm');
...
test('nabız: eşik içindeki taze koşuda alarm YOK (yanlış-pozitif freni)', () => {
  const simdi = new Date().toISOString();
  const kok = kurulum({ kosu: KOSU_SATIRI(simdi) });
  appendFileSync(join(kok, '00_pano', 'zarf-gunlugu.jsonl'),
    JSON.stringify({ surum: 1, ts: simdi, kosu: 'KOSU-E5', tip: 'nabiz' }) + '\n');
  kos(kok, 'nabiz.sh');
  assert.equal(gunluk(kok).filter((j) => j.tip === 'alarm').length, 0);
});

test('nabız: koşu kapandıysa uyanık-tutma savı BIRAKILIR (sav sızıntısı ayrı arızadır)', () => {
  const kok = kurulum();
  writeFileSync(join(kok, 'tools', 'sevk', '.caffeinate-pid'), '999999999\n');
  kos(kok, 'nabiz.sh');
  assert.ok(!existsSync(join(kok, 'tools', 'sevk', '.caffeinate-pid')), 'koşu yokken sav kaydı temizlenir');
});
...
test('file-guard: ajan haber/nabız betiklerini çağıramaz; sıradan komutlar serbest', () => {
  ...
  for (const k of ['bash tools/sevk/haber.sh --olay alarm', 'sh ./tools/sevk/nabiz.sh',
                   'echo x && bash tools/sevk/haber.sh --olay kosu-bitti']) {
    const r = g(k);
...
test('zarf-ekle: E5 tipleri kabul edilir, uydurma tip REDDEDİLİR (beyaz liste fail-closed)', () => {
  ...
  const yaz = (tip) => spawnSync('bash', [join(kok, 'tools', 'sevk', 'zarf-ekle.sh')], {
    ...
    input: JSON.stringify({ surum: 1, ts: '2026-07-28T10:00:00Z', kosu: 'K1', tip }) });
