...
2. **Bekçi kapıları**nı stack'e göre kur (ör. testin/derlemenin komutu; .NET'te `dotnet build` 0 + `dotnet test` yeşil). Bekçiyi **sıfırdan yaz**; kontrol kategorilerini (tavan/şema/tek-yazar/tazelik/koruma-hattı) stack'ine göre parametrele (hazır bir string-eşlemesi kopyalamaya çalışma). **Kök şeması kurulu-proje kökünü hedefler:** kurulumdan sonra kökte yalnız `CLAUDE.md`, `README.md`, `NASIL_KULLANILIR.md`, `.gitignore`, `.kurulum-tamam`, `.claude/` + OS klasörleri + `tools/` (kokpit ve guard KEEPER'ları burada yaşar) kalır. `GENESIS.md` kurulum artığıdır (G5'te `00_genesis/`e taşınır) — onu şemaya **kalıcı whitelist'leme.** **Ürün dosyaları da şemaya girer (kritik):** roller ilk kutuda kod yazacak — K-01 stack'inin ürün kod dosya/dizinleri **meşrudur ve baştan whitelist'lenir.** K-01 bu düzeni **ilan etsin** (ör. Node: `server.mjs` · `public/` · `package.json`, runtime `data.json` gitignore'da; .NET: `*.csproj` · `src/` · `bin`+`obj` gitignore'da) ve bekçi kök-şeması o alanı kapsasın. Yoksa roller kodu yazınca bekçi "beklenmeyen dosya" diye **yanlış SARI** basar.
   ...
   **Bekçi-tarifi kontratı (ürettiğin her bekçi/kanca script'i buna uyar; teslim öncesi fiilen doğrula):**
   ...
   - **(Kalite kontratı)** Script `set -euo pipefail` ile başlar · teslimden önce `bash -n <script>` sözdizimi kapısından geçer · boş-glob (`shopt -s nullglob`) ve boşluklu-yol (değişkenler her yerde tırnaklı) vakalarında patlamaz · **fail-closed öz-testi:** bekçi kendi içinde hata verirse sonuç KIRMIZI'dır, sessiz YEŞİL geçmez. Kurulumda bunu fiilen tetikle (bekçiyi bilerek bozuk girdiyle/eksik dosyayla koştur), KIRMIZI bastığı kanıtını `GENESIS_DURUM`a damgala.
   - **(Koruma-hattı — İKİNCİ hat)** Bekçi her koşuda iki şeyi denetler: (i) guard kablosu yerinde mi — `.claude/settings.json`da PreToolUse file-guard girdisi duruyor + `tools/guard/file-guard.sh` ve `korunan-yollar.txt` mevcut/okunur; değilse KIRMIZI. (ii) `korunan-yollar.txt`teki yolların commit dışı değişimi (`git status --porcelain` çıktısında korunan yola dokunan satır): `[SERT]` yolda kirlilik → KIRMIZI, `[SORULUR]` yolda → SARI (meşru olabilir; sahibe not). Amaç: kanca (ör. platform sürüm güncellemesinde) sessiz ölse bile tören yakalar — çift hat.
